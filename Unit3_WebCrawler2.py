# ------------------------------------------------------------
# --- UNIT 3 WEBCRAWLER FINAL CODE ---------------------------
# -------------------------------------------------------------

# -----------------------------------------------------------   
# --- SUBROUTINE - get_page - simulated procedure
...
def get_page(url):
    ...
    return ""
# -----------------------------------------------------------   
# --- SUBROUTINE - get_next_target
# --- INPUTS current contents of page to be searched
# --- RETURNS url - next url found in page
# ---         end_quote - location of end of found url
# --- CALLS nothing, processes the passed in page
# --- NOTES  
# -----------------------------------------------------------
...
# --- SUBROUTINE - union
# --- INPUTS tocrawl[list]
# ---        links found from get_all_links(page) as q[list]
# --- RETURNS updates tocrawl[list] which is live
# --- CALLS nothing appends the two lists
# --- NOTES  
# -----------------------------------------------------------
def union(tocrawl,q):
    for e in q:
...
# -----------------------------------------------------------   
# --- SUBROUTINE - get_all_links
# --- INPUTS page - latest webpage to crawl for links
# --- RETURNS links[list] list of all url's found
# --- CALLS get_next_target to find the next link
# --- NOTES  
# -----------------------------------------------------------
...
def get_all_links(page):
    ...
    return links
# -----------------------------------------------------------   
# --- SUBROUTINE - crawl_web - MAIN PROGRAM
# --- INPUTS seed - initial url to start crawl from
# --- RETURNS crawled[list] - list of all found urls
# --- CALLS get_all_links - to get all links on page
# ---       union - to append tocrawl[list] to know which
# ---               url's still need to be crawled
# --- NOTES When all the tocrawl links with ongoing updates
# ---       are exhausted and moved from tocrawl[list] to
# ---       crawled[list] the program ends and the final list
# ---       crawled[list] is returned.
# --- 5/1/17 - Modification added to limit the total number 
# ---          of pages identified on each page.  
# -----------------------------------------------------------
def crawl_web(seed, max_pages):
    ...
    while tocrawl:
        page = tocrawl.pop()
        if page not in crawled and len(crawled) < max_pages: # 5/1/17 update
...
# ----------  ANSWER -----------------------------------------
# ------------------------------------------------------------
#['http://xkcd.com/353', '/license.html', 'http://www.buttercupfestival.com/buttercupfestival.htm',
# 'http://thisisindexed.com/', 'http://wondermark.com/', 'http://achewood.com/',
# 'http://questionablecontent.net/', 'http://www.boltcity.com/copper/', 'http://pbfcomics.com/',
# 'http://www.asofterworld.com', 'http://www.qwantz.com', '/atom.xml', '/rss.xml', '/354/', 
# 'http://dynamic.xkcd.com/random/comic/', '/352/', '/1/', '/', 'http://forums.xkcd.com/', 
# '/about/', 'http://store.xkcd.com/', 'http://blag.xkcd.com/', 'http://xkcd.com/554', 
# 'http://buttersafe.com/', 'http://www.smbc-comics.com/', 'http://oglaf.com/', 
# 'http://threewordphrase.com/', '/555/', '/553/',
